...
class PointCloud:
    ...
    def shiftToOrigin(self):
        ...
        if len(points) > 0:
          ...
          min_pt,max_pt=np.zeros(3),np.zeros(3)
        newshift = (min_pt + max_pt) / 2
        self.points -= newshift
        self.shift += newshift
    ...
    def readFromFile(self,infile_name='testdata.dat'):
      self.points = np.loadtxt(infile_name,delimiter=',')

    def pointsCloseToLine(self,a,b,dx):
       t = (b * (self.points - a))
       d = (self.points - (a + (t*b)))
       return self.points[np.where(np.linalg.norm(d,axis=1) <= dx)]
    
    def removePoints(self,Y):
       for i in Y.points:self.points = self.points[np.where(1-np.all(self.points==i,1))]
       return self.points
